...
/**
 * Type-safe variant of Array.isArray that accepts both mutable and readonly arrays.
 *
 * Like typesafeIsArray but with a return type that includes readonly arrays,
 * making it useful for functions that need to accept both mutable arrays and
 * readonly tuples (e.g., from const assertions).
 *
 * @param {unknown} value - The value to check
 * @returns {value is unknown[] | readonly unknown[]} True if value is an array (mutable or readonly)
 * @example Basic usage
 * ```javascript
 * const mutableArray = [1, 2, 3];
 * const readonlyArray = [1, 2, 3] as const;
 *
 * if (typesafeIsReadonlyArray(mutableArray)) {
 *   // mutableArray is unknown[]
 *   console.log(mutableArray.length);
 * }
 *
 * if (typesafeIsReadonlyArray(readonlyArray)) {
 *   // readonlyArray is readonly unknown[]
 *   console.log(readonlyArray.length);
 * }
 * ```
 * @example Used internally by path helpers
 * ```javascript
 * function getValueByPath(obj, path) {
 *   const pathKeys = typesafeIsReadonlyArray(path)
 *     ? [...path]  // Create mutable copy
 *     : path.split('.');
 *   // Process pathKeys...
 * }
 * ```
 * @example Accepting both path formats
 * ```javascript
 * function processPath(path) {
 *   if (typesafeIsReadonlyArray(path)) {
 ...
 * processPath('foo.bar');
 * processPath(['foo', 'bar']);
 * processPath(['foo', 'bar'] as const);
 * ```
 * @see {@link typesafeIsArray} - Variant that only accepts mutable arrays
 * @see {@link isStringArray} - Checks for string[] specifically
 */
