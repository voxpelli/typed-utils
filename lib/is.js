 ...
 * @template {string} T
 * @param {unknown} key
 * @param {Record<T, true>} collection
 * @returns {key is T}
 */
export function isPartOfCollection (key, collection) {
  return !!key && typeof key === 'string' && Object.hasOwn(collection, key);
}
 ...
 */
export function isArrayOfLiteralType (value, type) {
  return (
    typesafeIsArray(value) &&
    value.every(item => isType(item, type))
  );
}
 ...
 */
export function isObjectValueType (obj, type) {
  return (
    isObject(obj) &&
    isArrayOfLiteralType(Object.values(obj), type)
  );
}
